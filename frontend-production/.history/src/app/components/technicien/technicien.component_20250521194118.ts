...
competencesUniques: number = 0;
techniciensSansMachine: number = 0;
  ...
  this.technicienService.getTechniciens().subscribe(data => {
    ...
    this.competencesUniques = new Set(data.map(t => t.competences)).size;
    this.techniciensSansMachine = data.filter(t => !t.machineAssignee).length;
  });
}
